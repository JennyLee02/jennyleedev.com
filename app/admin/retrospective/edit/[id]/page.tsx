...
export default function EditRetrospectivePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const extractMonthYearFromTitle = (title: string) => {
    const monthNames = [
      "January", "February", "March", "April", "May", "June",
      "July", "August", "September", "October", "November", "December"
    ]
    
    // Try to match pattern like "December 2023 Retrospective"
    const match = title.match(/^(\w+)\s+(\d{4})\s+Retrospective$/i)
    if (match) {
  ...
  // Parse existing content into structured form
  const parseContent = (content: string) => {
    const lines = content.split('\n')
    let currentSection = ''
    let highlights = ''
    let reflection = ''
    let achievements: ListItem[] = []
    let challenges: ListItem[] = []
    ...
    for (const line of lines) {
      ...
      if (trimmedLine.startsWith('## Month Overview')) {
        currentSection = 'highlights'
        captureText = true
        currentText = ''
        continue
      } else if (trimmedLine.includes('What Went Well') || trimmedLine.includes('🎉')) {
        if (captureText && currentSection === 'highlights') {
          highlights = currentText.trim()
        }
        currentSection = 'achievements'
        captureText = false
        continue
      } else if (trimmedLine.includes('Challenges Faced') || trimmedLine.includes('🚧')) {
        currentSection = 'challenges'
        captureText = false
        continue
      } else if (trimmedLine.includes('Key Learnings') || trimmedLine.includes('💡')) {
        currentSection = 'learnings'
        captureText = false
        continue
      } else if (trimmedLine.includes('Areas for Improvement') || trimmedLine.includes('🔧')) {
        currentSection = 'improvements'
        captureText = false
        continue
      } else if (trimmedLine.includes('Goals for Next Month') || trimmedLine.includes('🎯')) {
        currentSection = 'goals'
        captureText = false
        continue
      } else if (trimmedLine.includes('Final Reflection') || trimmedLine.includes('🤔')) {
      ...
      if (trimmedLine.startsWith('- ')) {
        ...
        switch (currentSection) {
          case 'achievements':
            currentItem = { text: item, image: "" }
            achievements.push(currentItem)
            break
          case 'challenges':
            currentItem = { text: item, image: "" }
            challenges.push(currentItem)
            break
          case 'learnings':
            learnings.push(item)
            currentItem = null
      ...
      } else if (trimmedLine.startsWith('![') && trimmedLine.includes('](') && currentItem) {
        // Extract image URL from markdown format ![Alt](url)
        const imageMatch = trimmedLine.match(/!\[.*?\]\((.*?)\)/)
        if (imageMatch) {
          currentItem.image = imageMatch[1]
        }
      } else if (captureText && trimmedLine && !trimmedLine.startsWith('#')) {
        // Capture text for highlights and reflection
        currentText += (currentText ? '\n' : '') + trimmedLine
      }
    }

    // Handle final reflection if it's the last section
    if (captureText && currentSection === 'reflection') {
      reflection = currentText.trim()
    }

    return {
      highlights,
      reflection,
      achievements: achievements.length > 0 ? achievements : [{ text: "", image: "" }],
      challenges: challenges.length > 0 ? challenges : [{ text: "", image: "" }],
  ...
  const fetchPost = async () => {
    try {
      ...
      if (response.ok) {
        const post: RetrospectivePost = await response.json()
        const { month, year } = extractMonthYearFromTitle(post.title)
        ...
        console.error("Failed to fetch retrospective post")
        router.push("/admin/retrospective")
      }
    } catch (error) {
      console.error("Error fetching retrospective post:", error)
      router.push("/admin/retrospective")
    } finally {
      setIsLoading(false)
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error("Failed to update retrospective post")
      }
    } catch (error) {
      console.error("Error updating retrospective post:", error)
    } finally {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="text-4xl font-bold mb-2">Edit Retrospective Post</h1>
          <p className="text-muted-foreground">
            Update your retrospective post using the structured form below.
          </p>
        </div>
